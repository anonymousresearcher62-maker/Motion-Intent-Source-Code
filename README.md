...
This app, and its work, largely centers on using ML on the edge or even on-device. As years progress, hardware gets more and more powerful thus enabling us to run much more intense models on our mobile devices. Obviously, however, they can only get so powerful before it reuqires a more capable computer. I used, by today's standards, an outdated (M1) iPad with 8GB of RAM.

That being said, it's possible to fit these models on an iPad (or any other smart device, rather - I Just chose an iPad) with enough compute capability. Android, for example, now has Gemma LLMs running on-device as well (https://www.datacamp.com/tutorial/gemma-3n). I chose to write this in a Playground, which limited me in some of the things I wanted to do because it doesn't have the same full capacity as Xcode -- rapid prototyping vs. connecting, compiling, enabling developer mode, restricted to 7 days per deployment, etc. Newer devices come with enough RAM to fit these models, though.

With enough resources for re-implementation, it'd be through Xcode to allow me to use models like Gemma MLX on iOS/iPad OS directly (the MLX runtime is fully supported for compilation). There are plenty of quantized versions of these models now available! 
The issue  with only 8GB of RAM is that it's reasonable to think it could eventually be killed by the OS. Gemma by itself uses at least ~4GB of memory on the low end, and OpenAI's model wouldn't be nearly possible  on a base-model iPad at all (at least 12 GB just to load). So, for now, division of responsibilities and using a capable machine that's still consumer level served the research perfectly. 
...
1. During the state mis/alignment events, there may be a crash. This happens most often if you move the whole device while in motion intent mode. Playgrounds doesn't have very good error reporting, which makes debugging complex apps slightly harder. In the event it doesn't crash, I added a "refresh" button to reset the headphone motion manager. To handle this in the future, we can add a permanent state change to reset the reference frames automatically.
   
2. Rarely, the compass direction doesn't reset properly and ends up cutting off the enrollment process early (usually two nodes instead of four).  There seems to be an error in the way the navigation stack lifecycle is maintained. It now has a "go-to-home" button which resets the enrollment to bypass this bug. 
...
We provide the mlmodelc ViT I modified to extract embeddings here, but you will need to zip it and download it into Playgrounds yourself. Or, if you want to use it in Xcode, you can disregard the next steps.
