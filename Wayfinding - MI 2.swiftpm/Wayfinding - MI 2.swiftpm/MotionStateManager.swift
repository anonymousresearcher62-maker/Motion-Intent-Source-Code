...
class MotionStateManager: MotionStateDelegate {
    ...
    func motionEventListener(_ pose: HeadPose) {
        ...
        if headPose != devicePose {
            ...
            guard let nd = floorGraph.nodeStorage[roomName] else {
                print("Error because of direction.")
                return
            }
            let currentState = nd.getNode(forDirection: locationManager.currentDirection)
            Task {
                var prompt = "Hello." 
                if headPose == .LookingRight || headPose == .LookingLeft {
                     prompt = "I am currently in Room \(roomName) facing wall \(currentState!.wallId), which is \(currentState!.direction). I am looking to my \(headPose.rawValue). Which wall in this room is to my \(headPose.rawValue)? Provide the scene description."
                }
                
                //analogous to looking "Backward" interactionless
                else if headPose == .LookingUp {
                    prompt = "I am currently in Room \(roomName) facing wall \(currentState!.wallId), which is \(currentState!.direction). What is behind me? Provide the scene description."
                }
                //analogous to looking "Forward" interactionless
                else if headPose == .LookingDown {
                    prompt = "I am currently in Room \(roomName) facing wall \(currentState!.wallId), which is \(currentState!.direction). What is directly in front of me? Provide the scene description."
                }
                print(prompt)
    ...
    func locationChangeListener(_ direction: String) {
        return // disbaled for now, causes too many issues
        ...
        if let headPose = currentHeadPose {
            ...
            if misaligned && (headPose == devicePose) && (headPose != .LookingForward){
                ...
                Task {
                    self.misaligned = false
                    var prompt = "I am currently in Room \(roomName) facing wall \(currentState.wallId), \(currentState.direction). What am I looking at?"
                    print(prompt)
                    var response = await llm?.makeAsyncRequest(withPrompt: prompt)
                    if let response = response {
                        self.responseBinding = response
