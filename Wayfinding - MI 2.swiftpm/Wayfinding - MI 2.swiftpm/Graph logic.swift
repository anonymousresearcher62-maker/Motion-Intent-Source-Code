...
class FloorGraph {
    ...
    // node storage; each "node" is a subgraph.
    ...
    func getGraphNL() -> String {
        ...
        if roomNames.count == 1 {
            nl += "Room \(roomNames[0]) Internal Details: "
            nl += nodeStorage[roomNames[0]]!.getGraphNL()
            nl += "\n---\n"
        }
        ...
        for roomN in roomNames {
            ...
            if let neigh = adjacency[roomN] {
                ...
                nl += "\(neigh.count) neighboring rooms:"
                for n in neigh {
                    nl += "Neighbor Room \(n.destination.0). \(n.destination.0)'s \(n.destination.1) connects to \(roomN)'s \(n.source.1)."
                }
                nl += "\n"
                nl += "Room \(roomN) Internal Details: "
